...
# Product: Withclass
# Context: src-gen/Withclass/ (same level as docker-compose.yml)
...
RUN mv eventmanager.product.withclass/*.jar ./jars/ 2>/dev/null || true && \
...
# Run the application
CMD ["java", "-cp", "jars/*:.", "eventmanager.product.withclass.Withclass"]
